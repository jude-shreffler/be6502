...
mos6502::mos6502(BusRead r, BusWrite w)
	: reset_A(0x00), reset_X(0x00), reset_Y(0x00), reset_sp(0xFD), reset_status(CONSTANT)
{
	Write = (BusWrite)w;
	Read = (BusRead)r;
	...
	while (cyclesRemaining > 0 && !illegalOpcode)
	{
		...
		cyclesRemaining -=
			cycleMethod == CYCLE_COUNT ? instr.cycles
									   /* cycleMethod == INST_COUNT */
									   : 1;
	}
}

void mos6502::RunEternally(uint64_t speed)
{
	uint8_t opcode;
	Instr instr;
	double secondsPerCycle = 1.0 / speed;
	...
	while (!illegalOpcode)
	{
		...
		while (end - start < std::chrono::duration<double>(secondsPerCycle))
		{
